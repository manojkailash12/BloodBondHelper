...
def show_blood_requests_for_donor():
    ...
    if compatible_requests:
        ...
        for i, request in enumerate(compatible_requests):
            with st.expander(f"Request #{request.get('id', i+1)} - {request.get('blood_group', 'Unknown')} - {request.get('urgency', 'Unknown')} Priority"):
                col1, col2 = st.columns([2, 1])
                
                with col1:
                    st.markdown(f"**Blood Group:** {request.get('blood_group', 'Unknown')}")
                    st.markdown(f"**Quantity Needed:** {request.get('quantity', 0)} ml")
                    st.markdown(f"**Urgency:** {request.get('urgency', 'Unknown')}")
                    st.markdown(f"**Required By:** {request.get('required_date', 'Not Provided')}")
                    st.markdown(f"**Reason:** {request.get('reason', 'Not Provided')}")
                    st.markdown(f"**Requester:** {request.get('requester', 'Unknown')}")

                    
                    request_date = datetime.fromisoformat(request.get('date', datetime.now().isoformat()))
                ...
                with col2:
                    ...
                    urgency_icon = urgency_colors.get(request.get('urgency', 'Unknown'), '⚪')
                    st.markdown(f"## {urgency_icon}")
                    st.markdown(f"**Priority:** {request.get('urgency', 'Unknown')}")
                ...
                with st.form(f"response_form_{request.get('id', i)}"):
                    ...
                    if response_type == "accept":
                        quantity_offered = st.number_input(
                            ...
                            min_value=100, 
                            max_value=request.get('quantity', 0), 
                            value=min(350, request.get('quantity', 0)),
                            step=50,
                            key=f"quantity_{i}"
                        )
                    else:
...
def show_request_responses():
    ...
    if responses:
        ...
        for request_id, data in requests_dict.items():
            ...
            with st.expander(f"Request #{request_id} - {request.get('blood_group', 'Unknown')} - {len(responses_list)} Response(s)"):
                ...
                with col1:
                    st.markdown(f"**Blood Group:** {request.get('blood_group', 'Unknown')}")
                    st.markdown(f"**Quantity:** {request.get('quantity', 0)} ml")
                    st.markdown(f"**Urgency:** {request.get('urgency', 'Unknown')}")
                
                with col2:
                    st.markdown(f"**Required By:** {request.get('required_date', 'Not Provided')}")
                    st.markdown(f"**Status:** {request.get('status', 'pending').title()}")
...
def get_blood_bank_names():
    try:
       ...
       # ← this must be defined in app.py or imported
